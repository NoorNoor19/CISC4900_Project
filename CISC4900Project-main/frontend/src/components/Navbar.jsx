...
const Navbar = () => {
  ...
  return (
    <>
      <header className='flex justify-between sticky top-0 p-4 bg-orange-500 shadow-sm items-center'>
        <h2 className='cursor-pointer  font-medium'>
          <Link to="/" className='text-white text-2xl'>any.Do </Link>
        ...
        <ul className='hidden md:flex gap-4 uppercase font-medium'>
          {authState.isLoggedIn ? (
            <>
              ...
              <li className=' bg-white text-orange-500 py-2 px-5  cursor-pointer hover:bg-white transition 
               rounded-3xl' onClick={handleLogoutClick}>Logout</li>
            </>
          ) : (
